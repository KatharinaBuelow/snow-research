...
def boxplot_single_exp_region(sel1r, plotdir, title,region, e, var):
    ...
    if var == 'sca':
        plt.xlim(0,100)
        plt.xlabel('snowcover [%]', fontsize=14)
    if var == 'snowday':
        plt.xlim(0,32)
        plt.xlabel('snowdays [days/month]', fontsize=14)
...
def boxplot_all_exp_region(sel1r, plotdir, title,region, t, var):
    ...
    sns.stripplot(x=var, y='height', data=sel1r, dodge=True, alpha=0.7, jitter=0.2, size=4, order=yorder, palette=my_cols, split=True, linewidth=1, edgecolor='gray', hue_order= hueorder,hue='experiment')

    ax=sns.boxplot(x=var, y='height', data=sel1r, whis=np.inf ,order=yorder, hue_order= hueorder,hue='experiment', palette=my_cols, boxprops=dict(alpha=0.7))
